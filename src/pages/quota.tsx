...
export function QuotaPage() {
  ...
  const filesQuery = useQuery({
    queryKey: QUERY_KEY,
    queryFn: () => api<{ files?: AuthFile[] }>("/v0/management/auth-files").then((r) => r.files ?? []),
  });

  const targets = useMemo(() => {
    return (filesQuery.data ?? []).filter((f) => supportsQuota(f) && !f.disabled);
  }, [filesQuery.data]);
  ...
  const upcomingResets = useMemo(() => {
    ...
    return list.sort((a, b) => a.resetAt - b.resetAt).slice(0, 5);
  }, [items]);
  ...
  return (
    <>
      <PageHeader
        title="额度查询"
        description="实时监控各 AI 平台 OAuth 账号及服务凭据的用量与配额恢复窗口"
        actions={
          <Button
            variant="outline"
            size="sm"
      ...
      <div className="grid gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Select value={sortMode} onValueChange={(v) => setSortMode(v as typeof sortMode)}>
              ...
              <SelectContent align="end">
                <SelectItem value="lowest">用尽/低额度优先</SelectItem>
        ...
        {filesQuery.isPending ? (
          ...
          <Card className="grid place-items-center py-16 text-center text-sm text-muted-foreground">
            {targets.length === 0 ? (
              <p>暂无支持额度查询的 OAuth 账号。支持 Codex、Claude、Devin、Kimi、Meta、xAI、Antigravity。</p>
            ) : (
              <p>没有找到符合当前筛选条件的账号额度信息</p>
            )}
          </Card>
        ) : (
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                    ...
                    <div className="mb-4 flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5">
                          <h3 className="truncate font-medium text-sm" title={item.name}>
                            {item.name}
                          </h3>
                        </div>
